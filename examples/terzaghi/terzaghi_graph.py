...
# ==============================================================================
f = h5py.File('./output/terzaghi_quad-poroelastic.h5','r')
...
#t_step_array[-1] -= 1
t_step_array = np.array([10, 71, 179, 358, 716])
...
ax.plot(pos_N[x_zero_pos,1], P_N[tstep, x_zero_pos], color=cm_numeric[1], label='Numerical, t* = ' + str(np.around(t_N[tstep],2) ) )
ax.plot(pos_N[x_zero_pos,1], P_exact_N[tstep, x_zero_pos], color=cm_analytic[1], marker='^', linestyle=' ',  label='Analytical, t* = ' + str(np.around(t_N[tstep],2) ) )

#t10
tstep = t_step_array[2]
ax.plot(pos_N[x_zero_pos,1], P_N[tstep, x_zero_pos], color=cm_numeric[2], label='Numerical, t* = ' + str(np.around(t_N[tstep],2) ) )
ax.plot(pos_N[x_zero_pos,1], P_exact_N[tstep, x_zero_pos], color=cm_analytic[2], marker='^', linestyle=' ',  label='Analytical, t* = ' + str(np.around(t_N[tstep],2) ) )

#t15
tstep = t_step_array[3]
ax.plot(pos_N[x_zero_pos,1], P_N[tstep, x_zero_pos], color=cm_numeric[3], label='Numerical, t* = ' + str(np.around(t_N[tstep],2) ) )
ax.plot(pos_N[x_zero_pos,1], P_exact_N[tstep, x_zero_pos], color=cm_analytic[3], marker='^', linestyle=' ',  label='Analytical, t* = ' + str(np.around(t_N[tstep],2) ) )

#t20
tstep = t_step_array[4]
ax.plot(pos_N[x_zero_pos,1], P_N[tstep, x_zero_pos], color=cm_numeric[4], label='Numerical, t* = ' + str(np.around(t_N[tstep],2) ) )
ax.plot(pos_N[x_zero_pos,1], P_exact_N[tstep, x_zero_pos], color=cm_analytic[4], marker='^', linestyle=' ',  label='Analytical, t* = ' + str(np.around(t_N[tstep],2) ) )


ax.grid()
ax.legend(loc='lower left')
ax.set(xlabel='Normalized Distance, y*', ylabel='Normalized Pressure, P*', title="Terzaghi's Problem: Pressure at x = 0")
fig.tight_layout()
...
#t10
tstep = t_step_array[2]
ax.plot(pos_N[x_zero_pos,1], U_N[tstep, x_zero_pos,1], color=cm_numeric[2], label='Numerical, t* = ' + str(np.around(t_N[tstep],2) ) )
ax.plot(pos_N[x_zero_pos,1], U_exact_N[tstep, x_zero_pos,1], color=cm_analytic[2], marker='^', linestyle=' ',  label='Analytical, t* = ' + str(np.around(t_N[tstep],2) ) )

#t15
tstep = t_step_array[3]
ax.plot(pos_N[x_zero_pos,1], U_N[tstep, x_zero_pos,1], color=cm_numeric[3], label='Numerical, t* = ' + str(np.around(t_N[tstep],2) ) )
ax.plot(pos_N[x_zero_pos,1], U_exact_N[tstep, x_zero_pos,1], color=cm_analytic[3], marker='^', linestyle=' ',  label='Analytical, t* = ' + str(np.around(t_N[tstep],2) ) )

#t20
tstep = t_step_array[4]
ax.plot(pos_N[x_zero_pos,1], U_N[tstep, x_zero_pos,1], color=cm_numeric[4], label='Numerical, t* = ' + str(np.around(t_N[tstep],2) ) )
ax.plot(pos_N[x_zero_pos,1], U_exact_N[tstep, x_zero_pos,1], color=cm_analytic[4], marker='^', linestyle=' ',  label='Analytical, t* = ' + str(np.around(t_N[tstep],2) ) )

ax.grid()
ax.legend(loc='best')
ax.set(xlabel='Normalized Distance, y*', ylabel='Normalized Displacement, U*', title="Terzaghi's Problem: Y Displacement at x = 0")
fig.tight_layout()
